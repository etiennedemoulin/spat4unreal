...
g(i)	=	hslider("[%i+1]Amplitude",1,0,20,0.001); // gain
r(i)	=	hslider("[%i+2][osc:/radius_%i 0.5 50][style:knob]Radius", 2, 1, 50, 0.01);// radius
t(i)	=	hslider("[%i+3][osc:/azimuth_%i 0 360][style:knob]Azimuth", 0, 0, 360, 0.1)*ma.PI/180; // azimuth
d(i)	=	hslider("[%i+4][osc:/elevation_%i -90 90][style:knob]Elevation", 0, -90, 90, 0.1)*ma.PI/180; // elevation

spherical(i)	=	1;

// Spherical restitution speaker layout radius r2 is needeed to stabilize near-field filters, see [1]
r2(i)	=	0.5; // louspeaker radius

r22(i) = r2(i) + (1-spherical(i))*(r(i)-r2(i)); // r2 is transformed to r when shperical(i) = 0 ==> spherical/plane wave selection;

source(i) = _*g(i)*r22(i)/r(i)<:par(m,M+1,nf(m,r(i),r22(i))<:par(i,2*m+1,_)):>yvec((M+1)^2,t(i),d(i));

//process	=	hgroup("",vgroup("Parameters",par(i,N,_<:selecteur(i))):>si.bus((M+1)^2):vgroup("[~]Outputs",outsvumeter));
process = hgroup("",par(i,N,_<:source(i)):>si.bus((M+1)^2):vgroup("[~]Outputs",outsvumeter));
